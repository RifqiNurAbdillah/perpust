...
`
	...
	fmt.Println("🔧 Mencoba memperbaiki Auto Increment users...")
...
func main() {
	...
	http.HandleFunc("/api/books/popular", popularBooksHandler)
	// static file
...
func sendEmailOTP(to, otp string) error {
	...
	// langsung kirim, jangan set send_at
	_, err := ms.Email.Send(ctx, message)
	if err != nil {
		return fmt.Errorf("MailerSend error: %w", err)
	}
...
func verifyOTPHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		if err != nil {
			...
			templates.ExecuteTemplate(w, "otp.html", data)
			return
		}
		...
		if err != nil {
			data := map[string]string{
				"Email": email,
				"Error": "Gagal update status verifikasi",
			}
			templates.ExecuteTemplate(w, "otp.html", data)
			return
		}
...
func listBooksHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 1. Ambil Parameter
	idParam := r.URL.Query().Get("id") // <--- PENTING: Tangkap ID
	...
	// ========================================================
	// LOGIKA KHUSUS: JIKA ADA ID, ABAIKAN PAGINATION & FILTER LAIN
	// ========================================================
	if idParam != "" {
		conditions = append(conditions, "id = ?")
		args = append(args, idParam)

		// Langsung gabungkan WHERE tanpa Limit/Offset
		baseQuery += " WHERE " + strings.Join(conditions, " AND ")

	} else {
		// ========================================================
		// LOGIKA NORMAL (LIST / SEARCH)
		// ========================================================
		...
		// --- PAGINATION (Hanya berlaku jika TIDAK mencari ID spesifik) ---
...
// Handler untuk mengambil buku terpopuler (Bookmark + Transaksi + History Ebook)
func popularBooksHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	// Query menghitung skor
	// Score = Jumlah Bookmark + Jumlah Transaksi (Pinjam) + Jumlah History (Baca)
	query := `
		SELECT 
			b.id, b.title, b.author, b.coverFile,
			(
				(SELECT COUNT(*) FROM bookmarks WHERE bookId = b.id) +
				(SELECT COUNT(*) FROM transactions WHERE book_id = b.id) +
				(SELECT COUNT(*) FROM ebook_history WHERE bookId = b.id)
			) AS popularity_score
		FROM books b
	...
	var popularBooks []map[string]interface{}
	for rows.Next() {
		var id int
		var title, author, coverFile sql.NullString
		var score int

		if err := rows.Scan(&id, &title, &author, &coverFile, &score); err != nil {
			continue
		}

		popularBooks = append(popularBooks, map[string]interface{}{
			"id":        id,
			"title":     title.String,
			"author":    author.String,
			"coverFile": coverFile.String,
			"score":     score, // Dikirim ke frontend jika ingin ditampilkan
		})
	}

	json.NewEncoder(w).Encode(popularBooks)
}
