...
async function loadBookDetail() {
    ...
    try {
        // PERBAIKAN: Tambahkan ?id=${bookId}
        // Ini memaksa backend mencari buku ini saja, walau dia buku lama.
        const res = await fetch(`/books?id=${bookId}`); 
        const books = await res.json();

        // Karena backend mengembalikan array, kita cek isinya
        ...
        // PERBAIKAN: Ambil index 0 (karena hasil pencarian ID pasti cuma 1)
        const book = books[0]; 
